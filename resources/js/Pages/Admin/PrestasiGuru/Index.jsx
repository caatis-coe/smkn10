...
function Index({ auth, datas, success }) {
    ...
    const deletePrestasiGuru = (prestasiSekolah) => {
        setSelectedData(prestasiSekolah);
        setIsConfirmOpen(true);
    }

    const handleConfirmDelete= () => {
        router.delete(route("prestasi-guru-db.destroy", selectedData.id));
                                ...
                                <table className='w-full text-sm text-left rtl:text-right
                                    ...
                                    <thead className='text-xs text-gray-700 uppercase bg-gray-100
                                        ...
                                        <tr className='text-nowrap'>
                                            ...
                                            <th className='px-5 py-5'>Year</th>
                                            ...
                                            <tr key={index} className='bg-white border-b 
                                                ...
                                                <td className='px-3 py-2 text-nowrap text-right'>
                                                    <div className='flex justify-end gap-x-2 items-center'>
                                                        <Link href={route("prestasi-guru-db.edit", data.id)}
                                                            className='text-blue-500 text-lg hover:text-blue-400 transition-all'
                                                        >
                                                            <MdEdit />
                                                        </Link>
                                                        <button
                                                            onClick={() => deletePrestasiGuru(data)}
